...
def onReuqestLogin(loginName, password, clientType, datas):
    ...
    返回：
        :return:Tuple，返回值分别为（错误码，真实账号名，密码，客户端类别，客户端提交的数据datas），如果没有任何需要扩展修改的则通常返回值为毁掉传入的值（KBEngine.SERVER_SUCCESS, loginName, password, clientType, datas）。o
    """
    pass
...
def onRequestCreateAccount(accountName, password, data):
    ...
    返回：
        :return: Tuple，返回值分别为（错误码，真实账号名，密码，客户端提交的数据datas），如果没有任何需要扩展修改的则通常返回值为毁掉传入的值（KBEngine.SERVER_SUCCESS, loginName, password, datas）。
    """
    pass
