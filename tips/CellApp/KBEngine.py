...
def addWatcher(path, getFunction, setFunction):
    ...
    功能说明：
        ...
        例：
        这个函数添加一个监视变量在"scripts/players"监视路径之下。
        函数countPlayers在观察者观察时被调用。
    @param path: 创建监视的路径。
...
class Entity:
    ...
    def setAoIRadius(self, radius, hyst=5):
        ...
        @param hyst:float，指定超过AOI区域的滞后区域的大小。
                           合理的设定滞后区域将能够降低AOI碰撞的敏感度从而提高CPU执行效率。
                           一个实体进入另一个实体的AOI必须跨越AOI半径区域，
                           但实体离开AOI区域则需要移出AOI半径区域包括滞后区域。
        @return: None
        """
        pass
