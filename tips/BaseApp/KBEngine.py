...
def createBaseFromDBID(entityType, dbID, callback):
    ...
    @param callback:这是一个可选的回调函数，当操作完成的时候它会被调用。
                    ...
                    如果操作成功，baseRef会是一个mailbox或者是新创建的Base实体的直接引用，
                    databaseID会是实体的数据库ID，无论该实体是否已经激活wasActive都会有所指示，
                    如果wasActive是True则baseRef是已经存在的实体的引用(已经从数据库检出)。
                    如果操作失败，baseRef会是None，databaseID会是0，wasActive会是False。
...
def createBaseLocally(entityType, *params):
    ...
    @param params:可选参数, 一个Python字典对象。
                  ...
                  如果这个键是一个Cell属性，它会被添加到Base实体的'cellData'属性，
...
def executeRawDatabaseCommand(command, callback, threadID, dbInterfaceName):
    ...
    @param callback:可选参数，带有命令执行结果的回调对象（比如说是一个函数）。
                    这个回调带有3个参数：结果集合，影响的行数与错误信息。
                    这个结果集合参数是一个行列表。
                    每一行是一个包含字段值的字符串列表。命令执行没有返回结果集合（比如说是DELETE命令），
                    或者命令执行有错误时这个结果集合为None。
                    这个数字是命令执行受影响的行数。
...
class Base:
    ...
    def createCellEntity(self, cellEntityMB):
        ...
        @param cellEntityMB: 一个可选的CellEntityMailBox参数，指定哪个空间里创建这个cell实体的。
                            只能使用一个直接的CellEntityMailBox。
                            如果你有一个实体的BaseMailbox，你不可以使用baseMailbox.cell传给这个函数。
    ...
    def createInNewSpace(self):
        ...
        功能说明：
            ...
            这个属性是一个字典，对应实体的.def文件里的默认值同时还包括用于表示
            实体位置和方向(roll, pitch, yaw)的"position", "direction" 和 "spaceID"。
        """
        pass
    ...
    def writeToDB(self, callback, shouldAutoLoad):
        ...
        @param callback:这个可选参数是当数据库操作完成后的回调函数。它有两个参数。
                        第一个是boolean类型标志成功或失败，
                        第二个是base实体。
